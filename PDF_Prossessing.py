# -*- coding: utf-8 -*-
"""
Created on Wed May 19 17:01:55 2021
...
# graph radar
    ...
    values += values[:1]
    ax.plot(angles, values, color="blue", linewidth=1, linestyle='solid', label="meilleur sprint")
    ax.fill(angles, values, 'b', alpha=0.1)
    # Ind1
    values = df.loc[0].drop('Sprint').values.flatten().tolist()
    values += values[:1]
    ax.plot(angles, values, color="red", linewidth=1, linestyle='solid', label="Sprint 6")
...
# Graph bar
    ...
    # plt.show()
    plt.savefig("Images/plotbar.png")

# PDF page 1
    packet = io.BytesIO()
    canvas = canvas.Canvas(packet, pagesize=letter)

    canvas.drawImage('Images/plot.png', 52, 270, width=280, height=230, preserveAspectRatio=False)
    canvas.drawImage('Images/plotbar.png', 290, 270, width=250, height=240, preserveAspectRatio=False)
    ...
    c.drawImage('Images/Bestand6_sprint.png', 52, 450, width=500, height=300, preserveAspectRatio=False)
    c.drawImage('Images/all_sprint.png', 52, 70, width=500, height=355, preserveAspectRatio=False)
    c.save()
    packet2.seek(0)
    ...
    outputStream = open("Results/Rapport_sprint_repetes_" + INFOS["nom_test"] + ".pdf", "wb")
